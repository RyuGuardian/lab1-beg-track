...
var animal = 'Code Monkey';

assert(animal, 'This passes.');
assert(animal === 'Happy', 'code monkeys are never happy.');
...
for (var i = 0; i < splitSentence1.length; i++) {

  splitSentence1[i] = 'chirp ';

}

sentence1 = splitSentence1.join('').trim() + '.';
...
while (i < splitSentence2.length) {
  ...
  splitSentence2[i] = 'chirp ';

  i++;
}

sentence2 = splitSentence2.join('').trim() + '.';
...
assert(animalExhibitStats.mostPopular, 'Where\'d Lucky go?');
